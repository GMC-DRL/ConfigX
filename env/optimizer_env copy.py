...
class Optimizer(gym.Env):
    ...
    def initialize(self, seed=None, algorithm_class=None):
        ...
        # -------------------------------- parameters -------------------------------- #
        self.Npop = np.random.choice(self.config.Npop, p=self.config.Npop_prob) if not self.config.alg_abla_mod else 1
        ...
        self.enable_bc = True # np.random.choice([True, False], p=[0.7, 0.3])
        self.pop_reduce = []
        ...
        for i in range(self.Npop):
            ...
            # self.n_component += 1
            if self.Npop > 1 and self.regroup < 1 and not self.config.alg_abla_mod:
        ...
        self.reg_id = None
        if self.regroup > 0 and self.Npop > 1 and not self.config.alg_abla_mod:
    ...
    def rand_op(self, step_op):
        ...
        if op_type == 0 or self.config.alg_abla_op:
    ...
    def one_step(self, logits, had_action=None):
        ...
        for ip in range(self.Npop):
            ...
            if self.strategy_mode == 'Given' and self.enable_bc:
                ...
                res = self.boundings[ip](None, subpops[ip], softmax=self.config.softmax, rng=self.rng)
            trail = res['result']
