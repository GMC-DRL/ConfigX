...
class Initialization(Uncontrollable):
    def __init__(self) -> None:
        super().__init__()
        self.id.append(1)
        self.topo_rule = ['Niching', 'Niching', 'Niching', 'GA_Crossover', 'DE_Mutation', 'PSO_update']
    ...
    def exec(self, population: Population, rng=None, seed=None):
        population.group = [self.init(population.NP, population.dim, population.Xmax, population.Xmin, rng, seed)]
...
class Niching(Uncontrollable):
    def __init__(self) -> None:
        super().__init__()
        self.id.append(2)
        self.topo_rule = ['GA_Crossover', 'DE_Mutation', 'PSO_update']
    ...
    def exec(self, population: Population, rng=None, seed=None):
        ...
        pop_id = self.nich(group, cost, rng=rng)
...
def get_xn(population, num_xn, use_archive=False, archive_id=[], united_id=[], rng=None):
    if rng is None:
        rng = np.random
    group = population.trail[population.process_ip]
    NP, dim = group.shape
...
class Conver_y(Restart):
    ...
    def check_restart(self, population: Population, rng=None):
        if rng is None:
            rng = np.random
        scost = np.sort(population.trail_cost[population.process_ip])[:int(self.Myeqs * population.trail[population.process_ip].shape[0])]
...
class Linear(Reduction):
    ...
    def exec(self, population: Population, rng=None):
        ...
        # step_ratio = population.FEs / population.MaxFEs
...
class gaussian(GA_Mutation):
    def __init__(self, ) -> None:
        super().__init__()
        self.config_space = {'sigma': {'type': 'float', 'range': [0, 1], 'default': 0.1}},
...
class polynomial(GA_Mutation):
    def __init__(self) -> None:
        super().__init__()
        self.config_space = {'n': {'type': 'discrete', 'range': np.arange(19)+2, 'default': 2}}
...
class sbx(GA_Crossover):
    def __init__(self) -> None:
        super().__init__()
        self.config_space = {
            'n': {'type': 'discrete', 'range': np.arange(19)+2, 'default': 2},
        }
...
class FDR_PSO(PSO_update):
    def __init__(self) -> None:
        ...
        self.config_space = {
                ...
                'c2': {'type': 'float', 'range': [0, 2], 'default': 2},
        }
...
class CLPSO(PSO_update):
    ...
    def exec(self, logits, population: Population, softmax=False, fixed_action=None, rng=None, had_action=None):
        ...
        while tournament.shape[-1] > 1:
            tournament[:tournament.shape[-1]//2] = np.maximum(population.trail_cost[population.process_ip][tournament[:tournament.shape[-1]//2]], population.trail_cost[population.process_ip][tournament[-(tournament.shape[-1]//2):]])
